...
public partial class HDevelopExport
{
  ...
  public void display_result (HObject ho_Image, HTuple hv_DLPreprocessParam, HTuple hv_DLModelHandle)
  {
    ...
    try
    {
      ...
      //╕∙╛▌╘д┤ж└э╡─▓╬╩¤┤ж└э╬к║╬╤╡┴╖╩▒║Є═м╥╗╣ц╕ё═╝╞м
      preprocess_dl_samples(hv_DLSample, hv_DLPreprocessParam);
      //╜л╘д┤ж└э╡─═╝╞м╡╝╚ы─г╨═╜°╨╨╫╘═╞╢╧гм▓в╩ф│Ў╜с╣√DLResultгм╒т╕Ў▓╬╩¤╖╟│г╓╪╥к
      hv_DLResult.Dispose();
      HOperatorSet.ApplyDlModel(hv_DLModelHandle, hv_DLSample, new HTuple(), out hv_DLResult);
      //╘┌╘н═╝╫ю╓╨╝ф╧╘╩╛├√╫╓║═╓├╨┼╢╚-╨д╦╒╗к ╖╜╖иг║╧╘╩╛╨┼╧в╬к├√│╞║═╓├╨┼╢╚гм╚╗║є╩╟═╝╧ё╫°▒ъ╧╡гм╘┘╛═╩╟╛▀╠х╡─╬╗╓├гм╒т└я╧╘╚╗╩╟╛╙╓╨╧╘╩╛гм╒т└я╥к┴Ї╥т╡─╩╟XY╫°▒ъ
      if (HDevWindowStack.IsOpen())
      {
        using (HDevDisposeHelper dh = new HDevDisposeHelper())
        {
        HOperatorSet.DispText(HDevWindowStack.GetActive(), ((((hv_DLResult.TupleGetDictTuple(
            "classification_class_names"))).TupleSelect(0))+" ")+(((hv_DLResult.TupleGetDictTuple(
            "classification_confidences"))).TupleSelect(0)), "image", hv_Height/2, 
            hv_Width/2, "green", new HTuple(), new HTuple());
        }
      }
  ...
  public void init (out HTuple hv_DLModelHandle, out HTuple hv_DLPreprocessParam)
  {
    ...
    try
    {
      //1 ╚б╡├╤╡┴╖║├╡──г╨═гм─г╨═┤╙DTL└я╡╝│Ўгм╖┼╓├╙┌═м╥╗─┐┬╝╧┬
      hv_DLModelHandle.Dispose();
      HOperatorSet.ReadDlModel("model.hdl", out hv_DLModelHandle);
      //┤╙─г╨═╡├╡╜╧р╙ж╡─╘д┤ж└э▓╬╩¤ги─у▓╔╙├DTL╤╡┴╖╩▒║Є╡─гм╚ч═╝╞м┤є╨бгм═и╡└╡╚гй
      hv_DLPreprocessParam.Dispose();
  ...
  private void action()
  {
    ...
    try
    {
      ...
      ho_Image.Dispose();
      HOperatorSet.ReadImage(out ho_Image, "D:/desk/winform/fruit_classify/fruit_classify/testimage/apple_2.jpg");
